...
4. `publish.yml` runs the gate again, then **stages** every package to npm (OIDC) and tags the release on `prod`. Delete the frozen branch once the tag exists: `git push origin --delete release/vX.Y.Z`.
