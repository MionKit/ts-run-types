...
The Vite plugin spawns this binary at JS test time and at build time — **build it before `pnpm test`**. The root `pretest` script runs [`scripts/check-go-binary.sh`](scripts/check-go-binary.sh) which auto-rebuilds when stale.
...
Every runtime command in both scripts now self-syncs prereqs (mirroring [`scripts/check-go-binary.sh`](scripts/check-go-binary.sh)): they rebuild the Go binary / Linux cross-binary / plugin dist / marker dist / podman image when any input is newer. Manual `pnpm run bench:prep` remains available for explicit refresh.
...
| JS plugin tests error spawning the resolver                    | `bin/ts-go-run-types` not built                                             | `pnpm run check:go-binary` or `go build -o bin/ts-go-run-types ./cmd/ts-go-run-types`.                                         |
...
| Marker package `tsc --build` fails with `Cannot find namespace 'Temporal'` | Pre-existing typecheck gap; runtime emit still happens                  | The bench prep uses `tsc -p tsconfig.json --noEmitOnError false` for this reason — `dist/formats/index.js` emits despite errors. |
