...
func TestBodyHash_Deterministic(t *testing.T) {
	got1 := BodyHash("mion", "asJSONString", "return JSON.stringify;")
	got2 := BodyHash("mion", "asJSONString", "return JSON.stringify;")
	if got1 != got2 {
...
func TestBodyHash_Length14(t *testing.T) {
	got := BodyHash("mion", "asJSONString", "return JSON.stringify;")
...
func TestBodyHash_FnNameAffectsHash(t *testing.T) {
	a := BodyHash("mion", "fnA", "return 1;")
	b := BodyHash("mion", "fnB", "return 1;")
	if a == b {
...
func TestBodyHash_NormalizesHorizontalWhitespace(t *testing.T) {
	// Mion collapses runs of spaces/tabs to single spaces, then trims.
	// Newlines are preserved.
	tight := BodyHash("mion", "fn", "return 1;")
	loose := BodyHash("mion", "fn", "  return    1;   ")
	if tight != loose {
...
func TestBodyHash_PreservesNewlines(t *testing.T) {
	// Mion's regex /[ \t]+/g doesn't touch newlines — they stay significant.
	noLines := BodyHash("mion", "fn", "const a = 1; const b = 2;")
	withLines := BodyHash("mion", "fn", "const a = 1;\nconst b = 2;")
