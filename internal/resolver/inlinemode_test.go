...
const pairedArraySource = `import {createValidate, getRunTypeId} from 'ts-runtypes';
...
const namedAliasArraySource = `import {createValidate, getRunTypeId} from 'ts-runtypes';
type Tags = string[];
...
func TestInlineMode_Default_CircularAliasThroughOptionalPropTerminates(t *testing.T) {
	source := `import {createJsonEncoder} from 'ts-runtypes';
