...
func typeFnEntry(key string, deps, softDeps []string) *entrymod.Entry {
	return &entrymod.Entry{Key: key, Kind: entrymod.KindTypeFn, FamilyTag: "pj", ArgsText: "'" + key + "'", Deps: deps, SoftDeps: softDeps}
}
...
func TestPruneUnit_NonTypeFnKindsAreRoots(t *testing.T) {
	graph := entrymod.Graph{}
	graph.Add(&entrymod.Entry{Key: "runtypes", Kind: entrymod.KindRunTypeBundle, ArgsText: "'runtypes'"})
	graph.Add(&entrymod.Entry{Key: "facade1", Kind: entrymod.KindRunTypeFacade, Deps: []string{"runtypes"}})
	graph.Add(&entrymod.Entry{Key: "pf/ns/fn", Kind: entrymod.KindPureFn, ArgsText: "'pf/ns/fn'", Deps: []string{"viaPureFn"}})
	graph.Add(&entrymod.Entry{Key: "stub1", Kind: entrymod.KindMissing})
