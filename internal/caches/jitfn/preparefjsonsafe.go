...
// because the wire shape (`[memberIndex, value]` union envelope,
// Date→ISO string, bigint→decimal string, etc.) is byte-for-byte
// identical to `prepareForJson + JSON.stringify`.
//
...
// emitUnionPrepareForJsonSafe mirrors the non-safe union emit but
// returns a NEW value at each clause. Uses the same `unionNeedsTuple`
// all-or-nothing decision so the wire shape stays compatible with
// the existing RestoreFromJson decoder.
func emitUnionPrepareForJsonSafe(rt *protocol.RunType, ctx *EmitContext, v string) JitCode {
	...
	for i, childRef := range children {
		...
		memberExpr, ok := safeChildExpr(childRef, v, ctx)
		if !ok {
			return JitCode{Code: "", Type: CodeNS}
		}
		...
		if needsTuple {
			resultExpr = "[" + strconv.Itoa(i) + "," + memberExpr + "]"
		} else {
			resultExpr = memberExpr
		}
		clause := "if (" + guard + ") return " + resultExpr + ";"
