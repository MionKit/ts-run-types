...
// (a) treat a builder call as a valid CompTimeArgs leaf and (b) terminate a
// CompTimeRunType schema-reference trace at the builder that created the
// run-type.
//
...
// any user wrapper that returns a `RunType<…>`. A name list would misclassify
// those and, worse, turn a legitimate `createIsTypeFor(temporal.instant())`
// into a spurious build error.
//
...
// IsBuilderCall reports whether call resolves to a value-first builder — a
// function declared in markerModule whose resolved return type is
// `RunType<…>`. Mirrors isSchemaFormFactory's resolved-signature walk
// (internal/resolver/scan.go) but inspects the return type instead of the
// function name.
func IsBuilderCall(typeChecker *checker.Checker, markerModule string, call *ast.Node) bool {
	...
	return isRunType(returnType, markerModule)
}
...
// propMod(), returning a {__propMod,…} carrier). Broader than IsBuilderCall —
// a user-module call is neither, so dynamic construction is still rejected.
// Each accepted call self-validates its own CompTimeArgs args on its own scan
// visit, so the leaf check STOPS here without recursing.
