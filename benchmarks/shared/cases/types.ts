...
// `createValidate` / `@mionjs/ts-go-run-types` imports, so a competitor that
// imports the shared suite never transitively pulls the marker package or the
// plugin — the ts-go-run-types validators live in the ts-go competitor module,
...
export interface SharedCase {
  ...
  /** ts-go-run-types is unsupported-at-root for this kind (its validator throws
