...
# (scripts, containers, CI, tests). check-env.sh reports it; .env.sample mirrors the
...
# --- docs website knobs (scripts/website.sh, scripts/podman-website.sh) ---
...
# --- benchmark knobs (scripts/benchmarks.sh) ---
...
RT_FUZZ_RACE|dev|-|enable the enrich race test (set by fuzz:race)
...
RT_AUDIT_ALIGNMENT|internal|-|Bench mode flag: emit alignment records instead of timing (set by audit:alignment)
