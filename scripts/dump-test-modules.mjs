...
// Vite-build-style snapshot under logs/build/:
//
...
// logs/build/ to inspect what the plugin produces.
...
const OUT_DIR = path.join(REPO_ROOT, 'logs/build');
...
async function main() {
  ...
  console.log(
    `dump-test-modules: ${rewriteCount} fixture${rewriteCount === 1 ? '' : 's'} rewritten ` +
