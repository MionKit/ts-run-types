# lib.sh - shared config + helpers for the containerized apps, sourced
# (never executed) by scripts/container/image.sh (image lifecycle), scripts/website/site.sh
# (run the docs site) and indirectly by scripts/website/bench-data/bench.sh's delegation.
#
...
# image.sh; the run commands live in site.sh; bench.sh delegates
# image ops to image.sh. ASCII-only by policy (macOS bash 3.2 mis-parses
...
# Repo-root .env loading (dev only) + the env-var registry, centralized in registry.sh.
RT_ENV_ROOT="$ROOT_DIR"
source "$(dirname "${BASH_SOURCE[0]}")/../env/registry.sh"

ENGINE="${RT_WEBSITE_ENGINE:-podman}"
# The single shared image. Built/published by image.sh; run by site.sh
# (at /app) and bench.sh (at /bench).
...
# the host tree is never written to; image.sh:clean drops them with the image.
VOL_NUXT="${CONTAINER_BASE}-nuxt"
VOL_DATA="${CONTAINER_BASE}-data"
VOL_CACHE="${CONTAINER_BASE}-cache"

# Error + exit, prefixed with the invoking script's name (site.sh / image.sh).
die() { echo "${0##*/}: $*" >&2; exit 1; }
