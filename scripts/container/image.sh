#!/usr/bin/env bash
# -----------------------------------------------------------------------------
# image.sh - own the lifecycle of the SINGLE shared podman image
# (container/website/Containerfile). That one image bakes BOTH dependency trees in
# separate dirs with separate node_modules:
#   /app    the Nuxt/Docus website deps   (run by scripts/website/site.sh)
#   /bench  the benchmark deps            (run by scripts/website/bench-data/bench.sh)
...
# push, pull, clean, lock. scripts/website/site.sh (run the site) and scripts/website/bench-data/bench.sh
# (run the benchmarks) both delegate image ops here. ASCII-only (macOS bash 3.2).
#
# Usage:
#   scripts/container/image.sh build-image   # build (or rebuild) the image, host-native
#   scripts/container/image.sh ensure        # make image ready: reuse local if it matches the published digest, else pull-or-build
#   scripts/container/image.sh login         # log in to GHCR (uses GHCR_PAT)
#   scripts/container/image.sh push          # build + push the multi-arch image to GHCR
#   scripts/container/image.sh pull          # pull the published image and tag it locally
#   scripts/container/image.sh lock          # regenerate _deps/pnpm-lock.yaml in-container
#   scripts/container/image.sh clean         # remove the image + named volumes
#
# Env overrides:
