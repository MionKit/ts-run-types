#!/usr/bin/env bash
# -----------------------------------------------------------------------------
# bench.sh - drive the validation benchmarks inside the shared (podman) image.
#
# The image is BUILT + PUBLISHED by scripts/container/image.sh: ONE merged image holds the
...
# and delegates image build/login/push/pull to image.sh, so the image has a
...
#
# Usage:
#   scripts/website/bench-data/bench.sh prep              # build the Go binary + JS packages (host)
#   scripts/website/bench-data/bench.sh build-image       # build the shared image (delegates to image.sh)
#   scripts/website/bench-data/bench.sh bench             # build + run EVERY competitor + aggregate
#   scripts/website/bench-data/bench.sh bench-one <name>  # build + run ONE competitor + aggregate
#   scripts/website/bench-data/bench.sh serialization     # ts-runtypes round-trip bench (+ formats), in-container
#   scripts/website/bench-data/bench.sh website-bench     # ALL website bench data in one shot (Node 26)
#   scripts/website/bench-data/bench.sh typecost          # per-competitor type-instantiation cost
#   scripts/website/bench-data/bench.sh compiletime       # tsgo build cost: strip / typecheck / full (ts-runtypes, typia)
#   scripts/website/bench-data/bench.sh transform-wire    # transform wire cost: 'go' vs 'edits' (size x density x file count)
#   scripts/website/bench-data/bench.sh capture-env       # write results/env.json (os / cpu / lib versions)
#   scripts/website/bench-data/bench.sh build [<name>]    # vite build only (all, or one competitor)
#   scripts/website/bench-data/bench.sh smoke             # quick verify: build every competitor's dist
#   scripts/website/bench-data/bench.sh audit             # cross-library validation alignment audit (analysis only)
#   scripts/website/bench-data/bench.sh shell             # debug shell inside the container
#   scripts/website/bench-data/bench.sh login             # log in to GHCR (delegates to image.sh)
#   scripts/website/bench-data/bench.sh push              # build + push the shared multi-arch image to GHCR
#   scripts/website/bench-data/bench.sh pull              # pull the shared published image and tag it locally
#   scripts/website/bench-data/bench.sh clean             # remove the typia .ttsc volume
#
...
#   GHCR_OWNER / GHCR_USER / GHCR_PAT  (see scripts/container/ghcr.sh).
#   RT_BENCH_CA_CERT  file/dir of extra CA certs (corporate/MITM proxy), forwarded to the build.
#   RT_BENCH_BASE_IMAGE / RT_BENCH_PNPM_VERSION  forwarded to the image.sh build.
...
# GHCR remote ref of the shared image (the same one image.sh publishes). Run
# commands pull it by default; RT_BENCH_USE_LOCAL=1 builds/uses a local image instead.
source "$SCRIPT_DIR/../../container/ghcr.sh"
...
# the bench env mapped onto image.sh's knobs, so the shared image has one owner.
run_manager() {
  (
...
# Image lifecycle is owned by image.sh (one merged image). Delegate to it.
...
mount_args() {
  [ -x "$LINUX_BIN" ] || die "missing $LINUX_BIN - run 'scripts/website/bench-data/bench.sh prep' first."
  [ -f "$MARKER_PKG/dist/index.js" ] || die "missing marker dist - run 'scripts/website/bench-data/bench.sh prep' first."
  [ -f "$PLUGIN_PKG/dist/index.js" ] || die "missing plugin dist - run 'scripts/website/bench-data/bench.sh prep' first."
  mkdir -p "$RESULTS_DIR"
...
# serialization bench (ts-runtypes-only round-trips). Runs gen-serialization.mjs
...
cmd_serialization() {
  ensure_prereqs
  [ -x "$LINUX_EXTRACT_BIN" ] || die "missing $LINUX_EXTRACT_BIN - run 'scripts/website/bench-data/bench.sh prep' first."
  [ -f "$MARKER_PKG/dist/index.js" ] || die "missing marker dist - run 'scripts/website/bench-data/bench.sh prep' first."
  [ -f "$PLUGIN_PKG/dist/index.js" ] || die "missing plugin dist - run 'scripts/website/bench-data/bench.sh prep' first."
  ...
  "$ENGINE" run --rm --init \
    ...
    -w "$tsgo" "$IMAGE" \
    sh -c 'node gen-serialization.mjs --suite serialization && node gen-serialization.mjs --suite format-serialization' </dev/null
}
...
cmd_website_bench() {
  ...
  ( cd "$ROOT_DIR" && node scripts/website/bench-data/gen-docs.mjs )
...
cmd_clean() {
  echo "==> removing the typia .ttsc volume (the shared image is managed by 'scripts/container/image.sh clean')"
