#!/usr/bin/env bash
# -----------------------------------------------------------------------------
# site.sh - RUN the isolated (podman) docs-website environment.
#
# The image lifecycle (build / push / pull / ensure / clean / lock) lives in
# scripts/container/image.sh; this script only RUNS the site in that shared image.
# Every run command first delegates `ensure` to image.sh so the image is
...
#
# Usage:
#   scripts/website/site.sh dev           # run the dev server with hot reload
#   scripts/website/site.sh dev --isAgent # detached agent dev server on RT_WEBSITE_AGENT_PORT
#                                    #   (3100); self-stops after RT_WEBSITE_AGENT_IDLE_SECONDS idle
#   scripts/website/site.sh build         # production build -> container/website/.output
#   scripts/website/site.sh generate      # static prerender -> container/website/.output/public
#   scripts/website/site.sh smoke         # quick verify: bg dev server + curl :3000 + stop
#   scripts/website/site.sh verify-docs   # verify code-import + twoslash render (curl/grep)
#   scripts/website/site.sh prep          # verify the repo context (packages/) is built
#   scripts/website/site.sh shell         # debug shell inside the container
#
# Image lifecycle is in scripts/container/image.sh (build-image | ensure | login |
...
#   RT_WEBSITE_USE_LOCAL=1   ensure uses a local image (maintainer/offline); forwarded to image.sh
...
cmd_verify_docs() {
  ...
  [ -n "$ex" ] || die "no examples found under $REPO_CONTEXT/packages/examples/src - run 'site.sh prep'"
...
main() {
  ...
  case "${1:-}" in
    ...
    shell)       cmd_shell ;;
    *) die "unknown command '${1:-}'. Try: dev | build | generate | smoke | verify-docs | shell  (image lifecycle: scripts/container/image.sh build-image|ensure|login|push|pull|lock|clean)" ;;
  esac
}
