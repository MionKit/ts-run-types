#!/usr/bin/env bash
# -----------------------------------------------------------------------------
# build.sh - the WHOLE docs-site publish pipeline in one command.
#
...
#
#   1. shared website+benchmark podman image   (image.sh ensure)
#   2. Go resolver binary + marker/plugin dist  (bench.sh prep)
#   3. suite-data -> public/suite-data/         (suite-data exporters, host)
#   4. all benchmark data -> bench-data/        (bench.sh website-bench)
#   5. playground assets -> public/playground-app/ (build-playground.sh, host)
#   6. static Nuxt build -> .output/public      (site.sh generate)
#
...
#
# Usage:
#   scripts/website/build.sh            # full pipeline, static artifact (generate)
#   scripts/website/build.sh build      # final stage = SSR/nitro build instead
#   scripts/website/build.sh --quick    # fast/preview benchmarks (noisy numbers)
#   scripts/website/build.sh --no-bench # SKIP stages 3+4; reuse the suite-data +
#                                         #   bench-data already on disk (rebuild
#                                         #   only). Errors if that data is absent.
#   RT_WEBSITE_USE_LOCAL=1 scripts/website/build.sh   # force a local image build
# -----------------------------------------------------------------------------
set -euo pipefail
...
source "$SCRIPT_DIR/../env/registry.sh"
...
for arg in "$@"; do
  case "$arg" in
    ...
    *) echo "website build: unknown arg '$arg' (want: [generate|build] [--quick] [--no-bench])" >&2; exit 2 ;;
  esac
done

# One USE_LOCAL knob across both child scripts: image.sh / site.sh
# read RT_WEBSITE_USE_LOCAL; bench.sh reads RT_BENCH_USE_LOCAL (its run_manager
...
require_bench_artifacts() {
  ...
  for dir in \
    ...
    if [ ! -d "$dir" ] || [ -z "$(find "$dir" -type f -name '*.json' -print -quit 2>/dev/null)" ]; then
      echo "website build: --no-bench needs '$dir' to already exist with data, but it is missing or empty." >&2
      missing=1
    fi
  done
  if [ "$missing" -ne 0 ]; then
    echo "website build: run a full 'pnpm rt website build' once to generate suite-data + bench-data, then re-run with --no-bench." >&2
    exit 1
  fi
}
...
if [ -n "$SKIP_BENCH" ]; then
  ...
  # Suite tables the validation/serialization pages fetch. The stage-2 prep above
  # already built the binary + dists these exporters need, so run them directly.
  ( cd "$SCRIPT_DIR/../.." \
  ...
  bash "$SCRIPT_DIR/bench-data/bench.sh" website-bench
fi
...
bash "$SCRIPT_DIR/site.sh" "$TARGET"
...
echo
echo "==> website build DONE (target: $TARGET${RT_BENCH_QUICK:+, quick benchmarks}${SKIP_BENCH:+, no-bench: reused suite+bench data})"
