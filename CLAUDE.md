...
- **JS workspace** at [packages/](packages/) — `@mionjs/ts-go-run-types` (marker + runtime helpers) and `vite-plugin-runtypes` (Vite plugin that spawns the binary, rewrites calls, serves the per-entry `virtual:runtypes/*` modules).
...
- Packages located under [packages/](packages/):
  ...
  - `vite-plugin-runtypes`: Vite plugin — spawns the Go binary, applies byte-offset rewrites, serves one `virtual:runtypes/<key>.js` module per cache entry
...
- **One ES module per cache entry.** A fn entry (`<fnHash>_<typeId>`, incl. JSON composites) or a RunType data node (`t_<typeId>`) is served as `virtual:runtypes/<key>.js`, a pure-data module exporting `entry` — the legacy skeleton `init()` / `rt()` positional args as an array with a family-tag slot at index 1, plus (data nodes only) a trailing gated `initEntry(rtUtils)` carrying the old footer (knot refs + runtime values like `BigInt(…)` / `globalThis.Date`). Renderers: [internal/compiled/typefns/entrymodule.go](internal/compiled/typefns/entrymodule.go) and [internal/compiled/runtype/entrymodule.go](internal/compiled/runtype/entrymodule.go); both are ⚠️ sync boundaries with the runtime registrar at [packages/ts-go-run-types/src/runtypes/registrar.ts](packages/ts-go-run-types/src/runtypes/registrar.ts).
- The scanner assembles each call site's **flattened transitive closure** ([internal/resolver/modules.go](internal/resolver/modules.go) — same-family deps, cross-family edges, composite primitives, data-node refs; leafs-first, roots last, dangling-dep cascade) onto `Site.Deps`. The rewriter injects `[typeId, fnHash, [rt1, …]]` (createX), `[typeId, [rt1, …]]` (graph-demand), or the bare id string (reflection-only) and APPENDS one import per closure key at EOF — ESM import hoisting makes the position irrelevant and keeps every byte offset valid.
- At runtime, `initDependencies` registers a closure two-pass (register-if-absent, then run fresh data nodes' `initEntry`), so every `useRunType`/`getRT` knot ref resolves, cycles included. Register-if-absent is HMR-correct because ids are structural and version-salted; modules are content-addressed and never invalidated.
- Entries are **deduplicated twice** in [internal/compiled/runtype/](internal/compiled/runtype/) — pointer identity AND structural id — and the runtime registry preserves the shared-singleton invariant across modules (the second registration of a shared node is skipped together with its links).
- **Never store parent-relative data on a canonical node**. Cache entries are shared singletons (one per structural id), so any field whose meaning depends on which parent referenced the node is silently wrong the moment that node appears under more than one parent. If a relationship is parent-scoped, store it on the **parent** (e.g. `RunType.UnionDiscriminators` lives on the union), or have the consumer build the back-link at walk time from a known root.
- The pureFns cache is the ONLY aggregate overlay left ([packages/ts-go-run-types/src/caches/pureFnsCache.ts](packages/ts-go-run-types/src/caches/pureFnsCache.ts) — skeleton + splice, as before). Disk cache format is **v5** (`<typeID>/<fnHash>.json`, the entry-array literal + prefix-carrying dep refs).

## Three injection markers + demand-driven function caches

There are **three** trailing-slot injection markers (all in [`packages/ts-go-run-types/src/markers.ts`](packages/ts-go-run-types/src/markers.ts)). Full design write-up: [`docs/DEMAND-DRIVEN-FN-CACHES.md`](docs/DEMAND-DRIVEN-FN-CACHES.md).

- `InjectRunTypeId<T>` — **bare reflection** sites (`getRunTypeId`, `reflectRunTypeId`). Injects a bare `"<typeId>"` string; no modules, no demand.
- `InjectRunTypeData<T>` — **graph-demand** sites (`createMockType`, the value-first schema builders). Injects `[typeId, [deps]]` in module mode — the deps are the imported `t_<id>` data-module tuples, registered through the runtime registrar before the graph lookup.
...
- `it` (validate) is **special** — a shared cross-family dependency, because the JSON/binary union encoders/decoders and `validationErrors` call `val_<member>` at runtime. In module mode this is just an import edge: closure assembly follows each entry's `CrossFamilyDeps` into the validate family, so a file that only serializes a union still imports the per-member `val_` modules its body references. (The old `CrossFamilyValRoots` aggregate-seeding machinery is gone.)
- **Adding a new RT function family:** add an entry to the [`internal/operations`](internal/operations/) registry (Name + FamilyTag + Axis + FnKey), add a row to `typefns.Families` ([`internal/compiled/typefns/families.go`](internal/compiled/typefns/families.go) — keep validate LAST) **and a `FAMILIES` row in the runtime registrar** ([`packages/ts-go-run-types/src/runtypes/registrar.ts`](packages/ts-go-run-types/src/runtypes/registrar.ts) — fnID, args shape, noop identity; the ⚠️ sync boundary that replaced the per-family skeletons), give its `createX` an `InjectTypeFnArgs<T, '<fnKey>'>` trailing param (plus a `CompTimeFnArgs` option slot if it has comptime options) and tuple-reading runtime. Cross-family references are just import edges now — closure assembly follows `CrossFamilyDeps` automatically.
