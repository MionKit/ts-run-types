...
- Pipeline is split across single-purpose packages under [ts-go-runtypes/internal/](ts-go-runtypes/internal/), organized by product area: **[ts-go-runtypes/internal/compiler/](ts-go-runtypes/internal/compiler/)** — source transformers (program, marker, builders, comptimeargs, resolver, sourcerewrite — rewrite + source-map: `Apply`/`ComputeEdits`/`ComposeMaps` — virtualmodules, batchcompile — the `--compile` tsc-like batch: overlay emit + map composition); **[ts-go-runtypes/internal/cachegen/](ts-go-runtypes/internal/cachegen/)** — cache generation (runtype + runtype/typeid, typefunctions + formats, purefunctions, operations, diskcache, hashid); **[ts-go-runtypes/internal/enrichment/](ts-go-runtypes/internal/enrichment/)** — FriendlyText/MockData codegen (astcheck, cldr, mirror); shared: protocol, constants, diag, textpos, jsquote, testfixtures — keep each focused, never introduce cross-package state.
...
- Tuple-layout sync boundary: [ts-go-runtypes/internal/compiler/virtualmodules/virtualmodules.go](ts-go-runtypes/internal/compiler/virtualmodules/virtualmodules.go) (assembler) and [packages/ts-runtypes/src/runtypes/entryTuple.ts](packages/ts-runtypes/src/runtypes/entryTuple.ts) (runtime). Constants from [ts-go-runtypes/internal/constants/constants.go](ts-go-runtypes/internal/constants/constants.go), mirrored via `pnpm run gen:ts-constants`.
...
- `InjectRunTypeId<T>` — reflection-only (`getRunTypeId` static + value-first forms, value-first RT builders, `createMockData`). Injects `"<typeId>"`; drives the `runTypes` reflection cache (1:1 on shape, no options).
...
- Disk cache format **v13**: keys embed fnHash; payload is the tuple `ArgsText` (default-valued slots — interior ones too — rendered as JS holes via `typefunctions.holeifyArgs`) + persisted `IsNoop` bit. Bump `diskcache.FormatVersion` whenever emitted bytes change shape (the version history in [format.go](ts-go-runtypes/internal/cachegen/diskcache/format.go) is the changelog).
- Plugin **`emitMode`** option (`code` | `functions` | `both`; binary `--emit-mode`) selects what each fn entry ships: `code` (default) body string (runtime rebuilds via `new Function`); `functions` live `createRTFn` closure; `both` ships both (CSP runtimes reading `.code`). Disk fingerprint ([ts-go-runtypes/internal/cachegen/diskcache/fingerprint.go](ts-go-runtypes/internal/cachegen/diskcache/fingerprint.go), tag v4) folds `emitMode` in so modes never cross-read.
