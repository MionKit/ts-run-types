...
- **Adding a new RT function family:** add an entry to the [`internal/operations`](internal/operations/) registry (Name + FamilyTag + Axis + FnKey), add a row to `typefns.Families` ([`internal/compiled/typefns/families.go`](internal/compiled/typefns/families.go) — keep validate LAST) plus a `familyRenders` row in [`internal/resolver/dispatch.go`](internal/resolver/dispatch.go), give its `createX` an `InjectTypeFnArgs<T, '<fnKey>'>` trailing param (plus a `CompTimeFnArgs` option slot if it has comptime options) and tuple-reading runtime. No `constants.CompFns` / `MigratedFamilies` steps anymore, and no per-family `XxxModule`/`AnyXxxSupported` wrappers — `FamilySpec.Render`/`FamilySpec.AnySupported` cover those. If anything else references its cache cross-family, it **cannot** be naively demand-scoped — follow the `it` precedent (cross-family edge collection) instead.
