...
- `InjectTypeFnArgs<T, Fn>` — **every `createX<T>()` factory** (`createIsType`, `createGetTypeErrors`, the `huk`/`suk`/`uke`/`uku`/`fmt` group, `createJsonEncoder`/`createJsonDecoder`, `createBinaryEncoder`/`createBinaryDecoder`). Injects a `["<typeId>", "<fnId>"]` tuple, where `fnId` is the precise compile-time function selector the scanner computes from `Fn` + the call-site `CompTimeArgs` literal (the `IsTypeOptions` variant suffix for `it`/`te`, the JSON strategy token for the encoder/decoder). The tuple is the complete demand: it tells the backend exactly what to emit AND gives the runtime the exact lookup key — no key re-derivation.

Function caches are **DEMAND-DRIVEN**: a family's cache contains only the types its own `createX` call sites request — gated by `constants.MigratedFamilies`, seeded from each `Site.FnId`, and closed transitively by `collectFamilyDemand` in [`internal/compiled/typefns/module.go`](internal/compiled/typefns/module.go). A `getRunTypeId`-only file emits **zero** function-cache entries.
...
- **Adding a new RT function family:** register it in `constants.CompFns` (with its base tag + comptime-arg axis), give its `createX` an `InjectTypeFnArgs<T, '<tag>'>` trailing param plus tuple-reading runtime, and add its tag to `constants.MigratedFamilies`. If anything else references its cache cross-family, it **cannot** be naively demand-scoped — follow the `it` precedent (cross-family edge collection) instead.
