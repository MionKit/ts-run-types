...
Runnable fixtures that demonstrate what `ts-go-run-types` extracts from real TypeScript source. Each folder contains an `input.ts`; run the CLI against it to produce a type-metadata JSON cache.
...
```bash
../../bin/ts-go-run-types --one-shot --tsconfig tsconfig.json --cwd $PWD <<EOF
