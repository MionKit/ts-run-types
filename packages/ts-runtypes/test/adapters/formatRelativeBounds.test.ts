...
//
// Each scenario is created with createValidate<TF.StringDate<{... min/max}>>()
...
import {createValidate, createMockType} from 'ts-runtypes';
...
describe('relative date bounds (now±P, date components only)', () => {
  it('min: now-P1Y — rejects dates more than a year ago, accepts within', () => {
    const validate = createValidate<TF.StringDate<{format: 'YYYY-MM-DD'; min: 'now-P1Y'}>>();
  ...
  it('max: now+P1M — accepts up to one calendar month ahead', () => {
    const validate = createValidate<TF.StringDate<{format: 'YYYY-MM-DD'; max: 'now+P1M'}>>();
  ...
  it('min+max window: now-P7D .. now+P7D', () => {
    const validate = createValidate<TF.StringDate<{format: 'YYYY-MM-DD'; min: 'now-P7D'; max: 'now+P7D'}>>();
  ...
  it('bare now as max — rejects any future date', () => {
    const validate = createValidate<TF.StringDate<{format: 'YYYY-MM-DD'; max: 'now'}>>();
...
describe('relative time bounds (now±P, time components only)', () => {
  ...
  it('min: now-PT1H — rejects times more than an hour before noon', () => {
    const validate = createValidate<TF.StringTime<{format: 'HH:mm'; min: 'now-PT1H'}>>();
  ...
  it('max: now+PT2H — accepts up to two hours after noon', () => {
    const validate = createValidate<TF.StringTime<{format: 'HH:mm'; max: 'now+PT2H'}>>();
...
describe('exclusive date/time bounds (gt/lt)', () => {
  it('TF.StringDate gt — excludes the bound day, accepts the next', () => {
    const validate = createValidate<TF.StringDate<{format: 'YYYY-MM-DD'; gt: '2020-01-01'}>>();
  ...
  it('TF.StringDate lt — excludes the bound day, accepts the prior', () => {
    const validate = createValidate<TF.StringDate<{format: 'YYYY-MM-DD'; lt: '2020-12-31'}>>();
  ...
  it('TF.StringDate min + lt combine (inclusive lower, exclusive upper)', () => {
    const validate = createValidate<TF.StringDate<{format: 'YYYY-MM-DD'; min: '2020-01-01'; lt: '2020-01-03'}>>();
  ...
  it('TF.Date (native) gt/lt — strict on both epoch edges', () => {
    const validate = createValidate<TF.Date<{gt: '2020-01-01T00:00:00'; lt: '2020-01-01T00:00:02'}>>();
