...
// `unplugin.<bundler>` from this instance). The unified hooks
// (resolveId / load / transform / buildStart / buildEnd) run on every
// bundler; the Vite-only config + HMR hooks ride the `vite` escape hatch.
export const unplugin = createUnplugin<PluginOptions | undefined>((rawOptions) => {
  ...
  return {
    ...
    // buildStart is unified across bundlers — the spawn point for every
    // bundler except Vite (whose configResolved spawns earlier to capture
    // its resolved root). ensureResolver is idempotent, so the Vite path's
    // later buildStart call is a no-op.
    buildStart() {
      ensureResolver();
    },
    ...
    async load(this: any, id: string) {
      ...
      if (source === undefined) {
        this.error?.(
          `runtypes-devtools: no entry module for "${basename}" — the import was injected by a marker rewrite, ` +
    ...
    async transform(this: any, code: string, id: string) {
      ...
      // lines despite the injected import block + bindings. fileResult.map is
      // our wire SourceMap (protocol.ts) — structurally a valid source map,
      // but typed with `sources: (string|null)[]` where the bundler source-map
      // input wants string[]; cast at the boundary (the runtime object is
      // consumed as-is across Vite/Rollup/webpack/etc.).
