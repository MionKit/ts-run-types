...
.rt-playground .rtpg-layout { display: grid; grid-template-columns: 1.2fr 0.8fr 1fr; gap: 1px; background: var(--rtpg-border); min-height: 345px; }
...
/* On-demand "View generated code" bar below the editor row. */
.rt-playground .rtpg-codegen-bar { display: flex; align-items: center; gap: 10px; padding: 10px 13px; border-top: 1px solid var(--rtpg-border); background: var(--rtpg-panel); }

/* Generated-functions section: one card per family (shown on demand). */
...
.rt-playground .rtpg-card-wide { grid-column: 1 / -1; }
...
.rt-playground .rtpg-card .rtpg-code { border: 0; border-radius: 0; margin: 0; max-height: 360px; }
