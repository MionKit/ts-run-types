import {getRunType, RunTypeKind, type RunType} from 'ts-runtypes';
...
function describe(rt: RunType): string {
  switch (rt.kind as number) {
    ...
    case RunTypeKind.string: return 'string';
    case RunTypeKind.number: return 'number';
    case RunTypeKind.boolean: return 'boolean';
    case RunTypeKind.bigint: return 'bigint';
    case RunTypeKind.symbol: return 'symbol';
    case RunTypeKind.null: return 'null';
    case RunTypeKind.undefined: return 'undefined';
    case RunTypeKind.void: return 'void';
    case RunTypeKind.never: return 'never';
    case RunTypeKind.any: return 'any';
    case RunTypeKind.unknown: return 'unknown';
    case RunTypeKind.object: return 'object';
    case RunTypeKind.regexp: return 'RegExp';
    case RunTypeKind.templateLiteral: return 'string'; // a `${...}` template
    case RunTypeKind.literal: return JSON.stringify(rt.literal); // the value itself
    case RunTypeKind.enum: return (rt.values as unknown[]).map((value) => JSON.stringify(value)).join(' | ');

    // Single-child kinds: recurse into `child`.
    case RunTypeKind.array: return `${describe(rt.child as RunType)}[]`;
    case RunTypeKind.promise: return `Promise<${describe(rt.child as RunType)}>`;
    case RunTypeKind.rest: return `...${describe(rt.child as RunType)}[]`;
    case RunTypeKind.tupleMember: return `${describe(rt.child as RunType)}${rt.optional ? '?' : ''}`;

    // Multi-child containers: recurse over `children`.
    case RunTypeKind.tuple: return `[${(rt.children as RunType[]).map(describe).join(', ')}]`;
    case RunTypeKind.union: return (rt.children as RunType[]).map(describe).join(' | ');
    case RunTypeKind.intersection: return (rt.children as RunType[]).map(describe).join(' & ');
    case RunTypeKind.objectLiteral: return `{ ${(rt.children as RunType[]).map(describe).join('; ')} }`;
