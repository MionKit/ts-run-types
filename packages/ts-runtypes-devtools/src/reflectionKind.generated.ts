// ============================================================================
//                            AUTO-GENERATED FILE
//                            DO NOT EDIT MANUALLY
// ============================================================================
//
// Generated by cmd/gen-run-type-kind from internal/protocol/protocol.go.
//
// To update: change the Go source, then run:
//
//     pnpm rtx core codegen kind
//
// which regenerates this file AND the marker-package mirror
// (packages/ts-runtypes/src/runTypeKind.ts) from the same protocol consts, so
// the two can never drift. The TestRunTypeKindFileInSync Go test and
// `pnpm rtx core codegen kind --check` (CI) both fail on any drift.
//
// The devtools plugin's dep-free mirror of the reflection RunType kind
// discriminators. Values match the Go binary's wire output byte-for-byte and
// stay in lockstep with RunTypeKind in @ts-runtypes/core (same source).
//
// ============================================================================

export enum ReflectionKind {
  never = 0,
  any = 1,
  unknown = 2,
  void = 3,
  object = 4,
  string = 5,
  number = 6,
  boolean = 7,
  symbol = 8,
  bigint = 9,
  null = 10,
  undefined = 11,
  regexp = 12,
  literal = 13,
  templateLiteral = 14,
  property = 15,
  method = 16,
  function = 17,
  parameter = 18,
  promise = 19,
  class = 20,
  typeParameter = 21,
  enum = 22,
  union = 23,
  intersection = 24,
  array = 25,
  tuple = 26,
  tupleMember = 27,
  enumMember = 28,
  rest = 29,
  objectLiteral = 30,
  indexSignature = 31,
  propertySignature = 32,
  methodSignature = 33,
  infer = 34,
  callSignature = 35,
}

// kindRef is our sentinel — not a reflection kind. Used in JSON to point at
// another type by id without inlining the referenced node.
export const KIND_REF = -1;
