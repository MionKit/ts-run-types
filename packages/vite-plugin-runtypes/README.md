...
Vite plugin that rewrites **mion runtypes** marker calls (`getTypeInfo`, `isType`, `router`) into cache lookups, driven by the [`ts-run-types`](../../README.md) Go resolver.
...
You must also build the `ts-run-types` binary from the parent repo and pass its path to the plugin.
...
export default defineConfig({
  plugins: [
    runtypes({
      binary: "./bin/ts-run-types", // built from the parent Go module
...
| ----------------- | -------------------------- | --------------------------------------------------------- |
| `binary`          | — (required)               | Path to the compiled `ts-run-types` Go binary.           |
