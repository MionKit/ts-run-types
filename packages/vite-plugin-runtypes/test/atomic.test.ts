...
describe("vite-plugin-runtypes / atomic round-trip", () => {
  ...
  runMaybe("string", async () => {
    expect((await getCache().then((c) => getType(c, "string.ts"))).kind).toBe(
      ReflectionKind.string,
    );
  });
  runMaybe("number", async () => {
    expect((await getCache().then((c) => getType(c, "number.ts"))).kind).toBe(
      ReflectionKind.number,
    );
  });
  runMaybe("boolean", async () => {
    expect((await getCache().then((c) => getType(c, "boolean.ts"))).kind).toBe(
      ReflectionKind.boolean,
    );
  });
  runMaybe("bigint", async () => {
    expect((await getCache().then((c) => getType(c, "bigint.ts"))).kind).toBe(
      ReflectionKind.bigint,
    );
  });
  runMaybe("symbol", async () => {
    expect((await getCache().then((c) => getType(c, "symbol.ts"))).kind).toBe(
      ReflectionKind.symbol,
    );
  });
  runMaybe("null", async () => {
    expect((await getCache().then((c) => getType(c, "null.ts"))).kind).toBe(
      ReflectionKind.null,
    );
  });
  runMaybe("undefined", async () => {
    expect((await getCache().then((c) => getType(c, "undefined.ts"))).kind).toBe(
      ReflectionKind.undefined,
    );
  });
  runMaybe("void", async () => {
    expect((await getCache().then((c) => getType(c, "void.ts"))).kind).toBe(
      ReflectionKind.void,
    );
  });
  runMaybe("any", async () => {
    expect((await getCache().then((c) => getType(c, "any.ts"))).kind).toBe(
      ReflectionKind.any,
    );
  });
  runMaybe("unknown", async () => {
    expect((await getCache().then((c) => getType(c, "unknown.ts"))).kind).toBe(
      ReflectionKind.unknown,
    );
  });
  runMaybe("never", async () => {
    expect((await getCache().then((c) => getType(c, "never.ts"))).kind).toBe(
      ReflectionKind.never,
    );
  });
  runMaybe("object primitive", async () => {
    expect((await getCache().then((c) => getType(c, "object.ts"))).kind).toBe(
