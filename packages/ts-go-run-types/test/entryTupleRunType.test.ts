...
// runtime registration: initFromTuple must walk the deps thunks, register
// each node from its tuple fields, and run the per-entry ini footers so ref
// slots resolve through the registry — including the self-referential cycle
// case the bundle's combined footer normally covers.
...
// Tuple layout (kind 0): [entryKind, deps, ini, id, kind, ...trimmed scalars].
...
describe('entryTuple / kind-0 per-node runtype registration (allModules)', () => {
  it('registers a node graph and patches refs through the per-entry inis', () => {
    ...
    const child: EntryTuple = runTypeTuple(childId, 5 /* number */, () => [child]);
  ...
  it('handles a self-referential cycle without TDZ or infinite recursion', () => {
    ...
    const cycle: EntryTuple = runTypeTuple(
      cycleId,
      30,
      () => [cycle],
      (rtu) => {
