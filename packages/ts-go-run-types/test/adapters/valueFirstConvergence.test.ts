...
// derived from an `object({...})` builder model resolves to the SAME precompiled
...
const StringFirst = object({
  short: string({maxLength: 5}),
  long: string({minLength: 3}),
  pick: string({allowedValues: {val: ['a', 'b']}}),
});
type StringFirstTF = {
...
const DateFirst = object({past: date({max: 'now'})});
...
const RegexFirst = object({slug: string({pattern: /^[a-z-]+$/})});
type RegexFirstTF = {slug: FormatString<{pattern: {source: '^[a-z-]+$'; flags: ''}}>};

// An `optional(...)` field converges with a type-first optional property.
const OptionalFirst = object({req: string({maxLength: 5}), opt: optional(number({min: 0}))});
...
const ScalarFirst = object({active: boolean(), count: bigint({min: 0n, max: 1000n})});
type ScalarFirstTF = {active: boolean; count: FormatBigInt<{min: 0n; max: 1000n}>};
const TemporalFirst = object({
  at: temporal.instant({min: '2020-01-01T00:00:00Z'}),
  day: temporal.plainDate({max: '2030-12-31'}),
});
