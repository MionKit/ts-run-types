...
//
// createBinaryEncoder returns the trimmed buffer (sliced to the bytes written),
// so `.byteLength` is exactly the on-wire size for a top-level value.

import {describe, expect, it} from 'vitest';
import {createBinaryEncoder} from '@mionjs/ts-go-run-types';
