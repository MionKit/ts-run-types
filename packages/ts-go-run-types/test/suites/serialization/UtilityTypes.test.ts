// serialization / UtilityTypes — every UTILITY_TYPES case run through the JSON round-trip strategies
...
describe('serialization / UtilityTypes', () => {
  for (const c of Object.values(UTILITY_TYPES) as SerializationCase[]) {
    it(`unsafe — ${c.title}`, () => {
      if (c.factoryThrows) {
