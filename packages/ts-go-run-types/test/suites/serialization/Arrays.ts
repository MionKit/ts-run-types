...
export const ARRAYS = {
  array: {
    title: 'Array',
    description: 'Root `string[]` round-trips identically across JSON and binary, with samples covering a populated array and the empty case.',
  ...
  array_date: {
    title: 'Date array',
    description: '`Date[]` encodes each element to an ISO string on the JSON wire and restores to a Date, while binary packs each as a fixed 8-byte epoch.',
  ...
  multi_dimensional: {
    ...
    description: 'Nested `string[][]` round-trips identically across JSON and binary, with samples mixing ragged inner arrays alongside empty inner and outer arrays.',
    mutateEncoder: () => createJsonEncoder<string[][]>(undefined, {strategy: 'mutate'}),
    cloneEncoder: () => createJsonEncoder<string[][]>(undefined, {strategy: 'clone'}),
    directEncoder: () => createJsonEncoder<string[][]>(undefined, {strategy: 'direct'}),
    stripDecoder: () => createJsonDecoder<string[][]>(),
  ...
  non_serializable_in_array: {
    ...
    description: '`symbol[]` should throw at RT-compile time per mion semantics because a non-serializable element propagates to the root.',
  ...
  array_circular: {
    ...
    description: 'Self-referential `type CircularArray = CircularArray[]` exercises recursive element walking via the value-first `RT.circular` builder and a deeply nested empty-array sample.',
    mutateEncoder: () => {
