...
pnpm run ts-runtypes:smoke                       # 2. Go binary + vite plugin wiring smoke
...
**2. `pnpm run ts-runtypes:smoke`** - end-to-end smoke for the Go resolver
binary + vite plugin wiring ([scripts/ts-runtypes-smoke.mjs](../../../scripts/ts-runtypes-smoke.mjs)).
...
(`prets-runtypes:smoke`) re-runs `check:builds`, which auto-rebuilds the Go
...
- **macOS** - supported: installs via Homebrew, manages the `podman machine` VM
  ...
  `RT_WEBSITE_POLL=1 pnpm run website:dev` (VM file-watch needs polling).
