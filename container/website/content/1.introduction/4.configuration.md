...
| `tsconfig` | `tsconfig.json` | Path to your tsconfig, relative to the project root. Set it on the bundler plugin, on the lint settings, or with the CLI `--tsconfig` flag; it is the one option that cannot live inside the tsconfig itself. The default applies when the file exists, and a config you name explicitly must load or the tool stops with an error. |
