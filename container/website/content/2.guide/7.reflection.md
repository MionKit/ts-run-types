...
Because the shape is regular, a full walk is just a switch over `kind` that recurses through those edges. Here is a printer that renders any node back to a TypeScript-like string. It is the same dispatch `createMockData` runs internally, one arm per kind: leaves return on the spot, single-child kinds recurse through `child`, containers through `children`, and callables through `parameters` and `return`.
