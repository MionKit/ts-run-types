...
`registerPureFnFactory` wants the `"namespace::name"` id written as a literal, which is perfect when you own the call site. It does mean a library cannot wrap it: the moment a wrapper builds the id from a user-supplied name, that id is no longer a literal and the build rejects it.

`registerAnonymousPureFn` is the wrappable twin. You pass the factory and nothing else. The identity comes from a content hash of the function body, which the compiler fills in for you at build time, so two identical helpers always resolve to the same entry (register the same helper in two places and you get one shared copy).
