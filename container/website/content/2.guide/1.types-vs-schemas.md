...
description: Describe a shape as a plain TypeScript type or with schema builders; both compile to the same thing.
toc: false
---
...
```ts
import {object, number, string, boolean} from 'ts-runtypes';
...
The same works for shared option presets. A `createValidate` or `createJsonEncoder` call can spread a preset and override individual keys inline (a key written after the spread wins).
...
const encodePost = createJsonEncoder<Post>(undefined, {...preset});
```
