...
Classes extending Error get one safety default: the stack property inherited from Error is never written to the wire, because a stack trace contains server file paths. The name and message are still written, so the error stays useful on the receiving side, and a class that declares its own stack property keeps it.
