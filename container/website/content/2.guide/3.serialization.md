...
`createPrepareForJson` and `createRestoreFromJson` are that pair. `prepare` turns a typed value into a JSON-safe value; `restore` turns a JSON-safe value back into the typed shape. The string encoder and decoder are built on exactly these.
...
::note
`createPrepareForJson` and `createRestoreFromJson` are the mutate and preserve strategies. The other strategies (clone, compact, and the single-pass stringify) have no factory of their own, but a framework can still recover them by name from a marker. See [recovering a function that has no factory](/guide/compiler-markers#recovering-a-function-that-has-no-factory).
::
