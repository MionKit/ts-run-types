...
The check is **pay-for-use**: types that can't cycle don't get wrapped at all, even with the global flag on. The guard's walker code is itself only bundled into an app that has at least one type that can cycle, so apps that never cycle carry none of it. And only real cycles trigger it. A value shared between two siblings walks through fine, because the guard only watches the current path down, not every object it ever saw.
