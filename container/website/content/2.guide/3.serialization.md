...
`createJsonEncoder` and `createJsonDecoder` give you a string. Sometimes you already own the JSON envelope (a framework that parses one request body and stringifies one response), so you want the transform at the value level, without an extra stringify and parse.
...
```ts
const encode = createBinaryEncoder<User>(undefined, {sizeStrategy: 'intoBuffer'});
const buffer = new ArrayBuffer(4096);
const view = encode(user, buffer); // a Uint8Array view into `buffer`, zero-copy
```

To pick a safe size for `initialSize` or `intoBuffer`, ask `createBinarySizer` for the exact byte count. It runs the same encoding as the encoder but writes nothing, so it allocates no buffer:
...
::note
You only reach for `intoBuffer` when you want to own the buffer in performance-sensitive code. For everyday encode/decode, the default `createBinaryEncoder` / `createBinaryDecoder` calls handle their own buffers.
::
...
Classes extending Error follow native JSON here. The members inherited from Error, its `name`, `message` and `stack`, ride the wire only when they are enumerable own properties of the value, exactly what `JSON.stringify` writes. A plain error subclass ships none of them, so a stack trace (which holds server file paths) never leaks by default. To put one on the wire, define it as an enumerable property on your class, either a class field or `Object.defineProperty` in the constructor. A framework error that sets its own `name` and `message` this way keeps its envelope. You can opt any of your own properties into the same rule with a `@nonEnumerable` JSDoc tag. A guarded property is treated as optional, so validators accept its absence.

::note
This only affects JSON and binary. `createValidate` always checks a class by its structural shape and never routes through the serializer. Without a registered serializer, the build falls back to the structural shape and emits a `CLS001` Warning pointing you here. If the automatic `new YourClass()` fails because the constructor needs arguments, decode throws a `CLS002` error telling you to register a `deserialize`.
::
...
::note
One gap the type cannot close: a property that rides the wire only when it is enumerable (anything inherited from Error, or one you tag with `@nonEnumerable`) still shows as present in `DataOnly<T>` when your type marks it required. A vanilla error round-trips to an empty object, yet `DataOnly<Error>` still lists `name` and `message`. The value stays safe to validate (validators treat these as optional), but the decoder's return type can over-promise them. If you depend on such a property, make it enumerable on your class (a class field, or `Object.defineProperty` in the constructor) so it actually rides the wire, or declare it optional.
::
