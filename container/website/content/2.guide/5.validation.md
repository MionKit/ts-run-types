...
| --- | --- |
| `createHasUnknownKeys` | Predicate that returns `true` if the value carries any undeclared key. Takes a compile-time `{runsAfterValidation: true}` option for a much faster variant when the value already passed `validate`. |
| `createCloneExactShape` | A proper deep clone of the declared shape — undeclared keys are dropped by construction, the input is never mutated, and `clone(x) !== x` for every object-typed value (a class keeps its prototype; only primitives and opaque handles pass through). |
