...
| `createCloneExactShape` | A proper deep clone of the declared shape — undeclared keys are dropped by construction, the input is never mutated, and the result shares nothing mutable with it (a class keeps its prototype; only immutable values are shared). |
