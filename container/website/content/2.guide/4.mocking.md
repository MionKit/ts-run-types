...
The `invalidLeafProbability` knob (0 to 1) steers where the bad value lands. Near the default of 0.85 it corrupts a single deep field, so the value stays realistic and only one thing is wrong. At 0 it replaces the whole value instead.
