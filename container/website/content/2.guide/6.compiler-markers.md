...
The value it returns is stable across releases, so you resolve it once instead of pinning a table that drifts on every version bump. It also follows compile-time options, so `getFnHash('val', {noLiterals: true})` and `getFnHash('jsonEncoder', {strategy: 'mutate'})` return the ids for those specific variants, which a plain family-to-id table could not tell apart.
