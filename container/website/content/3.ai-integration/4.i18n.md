---
title: Translations (i18n)
description: Translate friendly labels and messages per locale, with the source text as fallback.
toc: false
---

Your [`FriendlyType<T>`](/ai-integration/friendly-type) map already holds every label and error message for a type, written in one language. Translations start from that fact: **the friendly map you wrote is the source language**. There is no separate message catalog to maintain.

A translation is an optional file with the exact same shape, one per locale. Every string in it is optional too: anything left blank falls back to the source text, string by string. A half-translated app never breaks and never shows a raw key; the untranslated pieces simply render in the source language until someone fills them in.

## The workflow

List your locales once, in the same tsconfig plugin entry that holds the rest of your [configuration](/guide/configuration):

```json
{
  "compilerOptions": {
    "plugins": [
      {
        "name": "ts-runtypes",
        "i18n": {
          "sourceLocale": "en",
          "locales": ["es", "pl", "pt-BR"]
        }
      }
    ]
  }
}
```

From there it is the same loop as the rest of the [enrichment workflow](/ai-integration/workflow-and-commands): scaffold a locale, fill the blanks (yourself, or the agent), and keep it in sync as things change:

```bash
# scaffold the Polish translation files (create-only, existing files are left alone)
ts-runtypes gen --translate pl

# after a type or the source text changes: re-sync, keeping everything you wrote
ts-runtypes gen --translate pl --update

# every configured locale in one go
ts-runtypes gen --translate all --update

# CI gate: are the translations complete and up to date?
ts-runtypes check --translate all

# clear parked comments left behind by removed fields
ts-runtypes gen --translate pl --prune
```

`check --translate` reports a missing translation file, unfilled blanks, a translation that is out of date against its source map, parked leftovers awaiting a prune, and a format name that templates use but your formats module never defines. By default those are warnings and the command still succeeds; set `i18n.strict: true` to turn them into errors for CI. Rendering at runtime stays lenient either way.

## What a translation file looks like

Translations live under `runtypes/generated/i18n/<locale>/`, mirroring your source tree the same way the friendly files do. The locale is a folder name, so a regional tag like pt-BR works as-is:

```
runtypes/generated/
  friendly/models/user.ts      friendlyUser (the source language)
  mock/models/user.ts          mockUser (mock data is never translated)
  i18n/pl/models/user.ts       pl_friendlyUser (Polish)
  i18n/pt-BR/models/user.ts    pt_BR_friendlyUser (Brazilian Portuguese)
```

Each file exports one const per type, prefixed with the locale and typed `Translation<T>` (the same shape as `FriendlyType<T>`; the alias just states the intent). The scaffold is your friendly map with every string blanked out and marked `@todo`. It never copies your source text in as if it were already translated:

```ts
// runtypes/generated/i18n/pl/models/user.ts, scaffolded by `gen --translate pl`
import type { User } from '../../../../../models/user';
import type { Translation } from 'ts-runtypes';

export const pl_friendlyUser: Translation<User> = {
  $label: '', // @todo
  name: {
    $label: '', // @todo
    $errors: {
      type: '', // @todo
      minLength: { one: '', few: '', many: '', other: '' }, // @todo
    },
  },
};
```

Fill what you can, leave the rest blank:

```ts
export const pl_friendlyUser: Translation<User> = {
  $label: 'Konto użytkownika',
  name: {
    $label: 'Imię i nazwisko',
    $errors: {
      type: '', // still blank: this one keeps rendering in the source language
      minLength: {
        one: '$[label] musi mieć co najmniej $[val] znak',
        few: '$[label] musi mieć co najmniej $[val] znaki',
        many: '$[label] musi mieć co najmniej $[val] znaków',
        other: '$[label] musi mieć co najmniej $[val] znaku',
      },
    },
  },
};
```

All the [placeholder tokens](/ai-integration/friendly-type#the-placeholder-dsl) work exactly as they do in the source map.

Translations stay in sync the same way the source maps do. Run `gen --translate pl --update` after a change: a new message arrives as a fresh blank, a removed one is parked as a comment (cleared by `--prune`), and nothing you wrote is ever lost. Renaming a type in your code renames its consts across every locale too.

## Plurals

A message that carries a count often needs different wording at different counts. English gets away with two forms (1 character, 3 characters); Polish needs four, Arabic six. You should not have to know any of that, so the generator handles it.

Every count-carrying rule (`minLength`, `maxLength`, `min`, `max`, `lt`, `gt`) is scaffolded as a small object with exactly the plural forms that file's language uses, and you only fill in strings. An English map gets `one` and `other`; the Polish translation of the same rule gets `one`, `few`, `many` and `other`, as in the example above. Every other rule (`type`, `pattern`, and the rest) stays a single string:

```ts
// in the English source map
minLength: {
  one: '$[label] needs at least $[val] character',
  other: '$[label] needs at least $[val] characters',
},
```

At render time the right form is picked with the browser's own plural rules (`Intl.PluralRules`), per locale. `other` is the one required form and covers any form you leave out; unused forms can be pruned, and a plain string is still fine on a count-carrying rule when one wording covers every count.

::warning
**The count is the rule's limit, not what the user typed.** For a field with minLength 3, the plural form (and `$[val]`) follow the 3, never the length of the received text. That way "needs at least $[val] characters" always agrees with its own number. Write each form for the limit it will display.
::

One more subtlety handled for you: a plural is translated as a whole. A half-filled translated plural degrades to its own `other` form first, and only a fully blank one falls back to the source; the renderer never mixes one language's forms with another's inside a single message.

## Rendering translated messages

`createFriendlyI18n` is the locale-aware version of [`createFriendly`](/ai-integration/friendly-type). Hand it the source map plus the translations you want to bundle, and it returns the same renderer, with the same `label` and `errors` methods:

```ts
import { createFriendlyI18n } from 'ts-runtypes';
import { friendlyUser } from 'runtypes/generated/friendly/models/user';
import { es_friendlyUser } from 'runtypes/generated/i18n/es/models/user';
import { pl_friendlyUser } from 'runtypes/generated/i18n/pl/models/user';

const friendly = createFriendlyI18n<User>(friendlyUser, {
  locale: 'pl',
  translations: { es: es_friendlyUser, pl: pl_friendlyUser },
});

friendly.errors(getUserErrors(badInput));
// → Polish messages, falling back to your source text wherever a blank was left
```

Translations are plain imports, so only the locales you import end up in your bundle. Two details worth knowing:

**Live locale switching.** `locale` also accepts any object with a string `value` property, such as a Vue ref. It is read again on every render, so flipping it changes the language of the next `errors()` call with nothing to rebuild:

```ts
import { ref } from 'vue';

const locale = ref('en');
const friendly = createFriendlyI18n<User>(friendlyUser, {
  locale, // read again on every render
  translations: { pl: pl_friendlyUser },
});

locale.value = 'pl'; // the next errors() call renders in Polish
```

The renderer itself is not reactive: call `errors()` per render, or wrap the call in a `computed()`.

**Locale matching.** The active locale does not need an exact match. Ask for pt-BR with only a pt translation on hand and the pt file is used; the renderer tries the exact tag first, then trims it toward the base language, and falls back to the source map when nothing matches. The same logic is exported as `resolveLocale(locale, translations)` if you want to reuse it. A missing translation never throws.

If your source maps are not written in English, pass `sourceLocale` (default `en`, matching the tsconfig default) so plurals rendered from the source map use the right rules.

## Formatting numbers and dates

Templates can format a value through the browser's `Intl` formatters with a three-part token: `$[val:number:currency]` reads as "the value, formatted as a number, using the named format currency". The named formats are declared once per locale, in a small module:

```ts
// runtypes/i18n.formats.ts
import type { NamedFormats } from 'ts-runtypes';

const formats: Record<string, NamedFormats> = {
  en: { number: { currency: { style: 'currency', currency: 'USD' } } },
  pl: { number: { currency: { style: 'currency', currency: 'PLN' } } },
};

export default formats;
```

Pass it to the renderer, and name it under `i18n.formats` in tsconfig so `check --translate` can confirm every format name your templates mention actually exists:

```ts
import formats from './i18n.formats';

const friendly = createFriendlyI18n<User>(friendlyUser, {
  locale: 'pl',
  translations: { pl: pl_friendlyUser },
  formats,
});
```

Then any template can use it:

```ts
max: { other: '$[label] must stay under $[val:number:currency]' },
```

Four kinds are available: `number`, `date`, `relativeTime` and `list` (`$[val:list:or]` renders a list-valued limit as "a, b, or c"). A `relativeTime` format also declares its unit (days, hours) next to its `Intl` options, since the token has no room for it. An unknown token, kind or format name is simply left as-is in the output, and a literal colon in prose (a ratio like 3:1) is never mistaken for a token.
