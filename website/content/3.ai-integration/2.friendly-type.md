---
title: FriendlyType
description: Human-readable labels and error messages for a type — author them once, then render validation errors into messages people can actually read.
toc: false
---

`FriendlyType<T>` is a human-readable map for a type — a **label** and **error messages** for each field. You author it once, [commit](/ai-integration/workflow-and-commands) it next to your type, and the compiler checks it against that type on every build. Use it to turn validation errors into messages people can actually read.
