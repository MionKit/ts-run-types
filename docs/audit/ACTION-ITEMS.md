...
- [x] **T6b** — extend mock/test activation to the **11 NUMBER/BIGINT** format `it.todo`s in `formatMockType.test.ts` (number: max/min/lt/gt/float/multipleOf; bigint: max/min/lt/gt/multipleOf). The mock generators (`mockNumberFormat.ts` / `mockBigIntFormat.ts`) were already implemented in T6; this wires the paired `createMockType` thunks into the suite and converts the stubs to live `it()`s. `formatMockType` NUMBER/BIGINT `it.todo`s → 0; full suite 0 todo.
- [x] **T7** — custom class serializer/deserializer registry (`registerClassSerializer(name, {serialize, deserialize})`). Consulted by JSON (`pj`/`pjs`/`pjsp`/`rj`/`sj`) + binary (`tb`/`fb`) for plain user classes (`SubKindNone`): registered → custom serialize↔JSON-ready-data / deserialize (binary reuses the string encoder over `JSON.stringify`); unregistered → structural plain-object fallback + `CLS001` build-time Warning. isType/getTypeErrors + builtins untouched. Round-trip suite extended to 13 tests (root JSON+binary static/reflect, unregistered fallback, registry isolation, `unregisterClassSerializer`, last-write-wins override, empty-name throw, nested class-as-property JSON+binary, class-as-array-element JSON).

**All planned tasks done.** No remaining optional items.
