...
4. Add the messages to `packages/ts-go-run-types/src/runtypes/diagnosticMessages.ts`.
...
3. Add the family's codes in `internal/diag/codes_runtype.go` and `packages/ts-go-run-types/src/runtypes/diagnosticMessages.ts`.
...
Override `packages/ts-go-run-types/src/runtypes/diagnosticMessages.ts` at build time (e.g. via a Vite plugin alias). The codes are stable; the messages are what gets localised.
