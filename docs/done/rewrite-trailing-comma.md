...
> _Resurfaced historical doc. This bug has since been FIXED (the fix survived the port of the rewrite step to the Go `transform` package); the "not fixed" status below is historical. The `vite-plugin-runtypes` plugin referenced below is now `ts-runtypes-devtools`._
