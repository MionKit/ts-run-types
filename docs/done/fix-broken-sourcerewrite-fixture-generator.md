...
The `TestApply_Fixtures` fixtures under [ts-go-runtypes/internal/compiler/sourcerewrite/testdata/](../../ts-go-runtypes/internal/compiler/sourcerewrite/testdata/) (`*.json`) can no longer be regenerated by their generator. `gen_golden.mjs` is **doubly stale** — both breakages predate the change that surfaced them (the `createX` → `createXFn` rename PR, which had to work around it):
...
[`ts-go-runtypes/cmd/gen-sourcerewrite-fixtures/main.go`](../../ts-go-runtypes/cmd/gen-sourcerewrite-fixtures/main.go),
...
exactly what `TestApply_Fixtures` / `TestApply_ExtraDiff` / `TestComputeEdits_MatchesApply` call — so
...
**What the suite is now.** With the independent JS oracle deleted, `TestApply_Fixtures` is a
...
**Done-when status:**
- ✅ Generator runs (`go run ./cmd/gen-sourcerewrite-fixtures`, CWD-independent via `runtime.Caller`)
