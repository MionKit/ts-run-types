...
# `sourcerewrite/testdata/gen_golden.mjs` is broken — source-map fixtures can't be regenerated

## Intent

The `TestApply_SourceMapFixtures` fixtures under [ts-go-runtypes/internal/compiler/sourcerewrite/testdata/](../../ts-go-runtypes/internal/compiler/sourcerewrite/testdata/) (`*.json`) can no longer be regenerated by their generator. `gen_golden.mjs` is **doubly stale** — both breakages predate the change that surfaced them (the `createX` → `createXFn` rename PR, which had to work around it):
...
Impact: the source-map fixtures are effectively frozen. Any future change to the rewrite / source-map logic can't refresh them, and a change that touches the synthetic fixture source (identifier text) breaks the baked UTF-8 byte offsets with no regeneration path. The `createXFn` PR handled this by **leaving the `testdata/` fixtures at their committed synthetic names** (the callee identifier is immaterial to the byte-offset/sourcemap mechanics under test) and filing this.
...
[`ts-go-runtypes/cmd/gen-sourcemap-fixtures/main.go`](../../ts-go-runtypes/cmd/gen-sourcemap-fixtures/main.go),
...
exactly what `TestApply_SourceMapFixtures` / `TestApply_ExtraDiff` / `TestComputeEdits_MatchesApply` call — so
...
**What the suite is now.** With the independent JS oracle deleted, `TestApply_SourceMapFixtures` is a
...
**Done-when status:**
- ✅ Generator runs (`go run ./cmd/gen-sourcemap-fixtures`, CWD-independent via `runtime.Caller`)
