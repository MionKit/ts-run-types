...
- **`@types/node` 24.12.2 → 26.x** once a 26.x release ages past the 30-day `minimumReleaseAge`
  policy (every 26.x was published after 2026-06-19, so all were too new at ship time). Node 24
  types are a safe subset of the Node 26 runtime in the meantime.
