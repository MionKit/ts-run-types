...
- `rewrite.ts` — single function: for each file, calls `scanFiles`, then applies the returned sites as **byte-offset** insertions plus one deduped single-line import block at offset 0 (`import {__rt_<key>} from 'virtual:rt/<key>.js';` per referenced entry — every entry module exports under its binding name, so clauses never rename). Edits go through the in-house `EditBuffer` (`edit-buffer.ts`), so the rewrite also returns a source map (original lines/columns survive the injection). tsgo positions are UTF-8 byte offsets while the `EditBuffer` indexes UTF-16 code units, so every resolver offset is converted via `makeByteToChar` first — raw JS string math would skew on any multibyte character (e.g. em-dashes in comments).
- `edit-buffer.ts` — the from-scratch string-editor + source-map generator that replaced the `magic-string` dependency, so the published plugin has **no runtime dependencies**. It implements only the slice the rewrite uses (`appendLeft`/`update`/`prepend`/`toString`/`generateMap`) and leans on three rewrite-specific invariants: edits are always against original coordinates (single render pass, application order irrelevant), edits never overlap (asserted, not conflict-resolved), and only the prepended import block adds newlines. The generated map matches magic-string's `hires: 'boundary'` segmentation (a VLQ segment at each word/non-word boundary of unedited runs, none inside inserted text) so Vite's composite-map chain is byte-for-byte equivalent.
- `index.ts` — Vite plugin glue. Short-circuits any file that doesn't reference the marker module (or `registerPureFnFactory`) as a cheap pre-filter. `resolveId`/`load` serve `virtual:rt/<key>.js` bodies verbatim from the `entryModules` map on the resolver's `dump` response — the Go side is the single renderer. The `moduleMode` option (forwarded as `--module-mode`) selects the grouping: `default` (per-entry fn modules + the runtype data bundle), `allSingle` (one bundle module per fn family + `pf` bundle + facades folded into the runtypes bundle — fewest modules), or `allModules` (per-node runtype modules, the pre-bundle layout).
...
The validators / encoders are straight-line walkers with no cycle protection: a runtime **value** that contains a reference cycle (`a.next = a`) makes them recurse until the stack overflows. `setCircularCheck(true)` ([`src/runtypes/circular.ts`](../packages/ts-go-run-types/src/runtypes/circular.ts)) arms a guard on the four live-object families — `createValidate`, `createGetValidationErrors`, `createJsonEncoder`, `createBinaryEncoder` (decoders take serialized input, which can't cycle). Off by default; a per-call `{checkCircular: true}` option on each factory overrides the global flag for that one instance. `checkCircular` is **runtime-only** — it is deliberately not one of the Go scanner's `ValidateOptions` and the JSON axis hashes only `strategy`, so it never folds into the `fnHash` (a checking and a plain factory for the same `T` share one compiled entry; pinned by `TestCheckCircularExcludedFromFnHash`).
