...
internal/compiler/resolver/               op dispatch (scanFiles, dump, …); AST call-walk (walk.go + scan.go)
...
internal/diagnostics/                   diagnostic codes (codes_*.go) + the single wording source (messages.go)
internal/enrichment/                 FriendlyText/MockData enrichment: checkers, mirror emit/reconcile/hygiene, astcheck/
internal/cachegen/diskcache/             persistent disk cache (FormatVersion, fingerprinting)
internal/compiler/virtualmodules/      per-entry virtual-module assembly (ordering, imports, stubs)
internal/cachegen/hashid/                 structural-id hashing (quickHash rolling hash → short ids)
...
- **Every cache entry is its own virtual module** (default mode) — `virtual:rt/<key>.js` (`<fnHash>_<typeId>` for function entries and JSON composites, `pf/<ns>/<fn>` for pure fns) — EXCEPT runtype nodes, which ride as headless ROWS of THE single data bundle `virtual:rt/runtypes.js` (tuple kind 4: content-hash key, rows array, a parallel `rels` array wiring each row's ref slots by ROW INDEX, and a residual `ini` footer for expression-specials only — `classType`, bigint/symbol literals, `formatAnnotation`), with a tiny per-reflection-root facade `virtual:rt/<rootId>.js` (kind 5: imports the bundle, carries the root id) so the rewrite's binding-only injection is unchanged. Each node row exists exactly once app-wide, and runtype emission is demand-driven on REFLECTION sites — a createX-only file emits zero runtype modules. Every module exports ONE positional tuple under its BINDING NAME (`__rt_<key>`, identifier-escaped — the same name in the export, every import clause, and the call-site binding; imports never rename): `[kindOrFamilyTag, depsThunk|hole, ini|hole, …legacy positional args]`. Absent slots are JS array HOLES, not a `u`/`undefined` alias (the runtime reads tuples by index, so a hole is indistinguishable from undefined); there is no `const u=undefined;` line, and every default-valued fn-tuple slot renders as a hole even when interior (a later non-default slot — a live factory, the tb size estimate — no longer forces the earlier defaults to be spelled out). The assembler lives in [internal/compiler/virtualmodules/virtualmodules.go](../internal/compiler/virtualmodules/virtualmodules.go); the runtime consumer in [packages/ts-runtypes/src/runtypes/entryTuple.ts](../packages/ts-runtypes/src/runtypes/entryTuple.ts) — those two files are the tuple-layout sync boundary. Constants (virtual prefix, binding prefix) come from [internal/constants/constants.go](../internal/constants/constants.go) (mirrored to TS via `pnpm run gen:ts-constants`).
...
- **Builtin classes project atomically** — `Date` / `Map` / `Set` / `RegExp` / Temporal / the non-serializable set stop at subKind + classRef (+ Map/Set element `Arguments`); their lib members are never walked or interned (`projectClass` in [internal/cachegen/runtype/serialize.go](../internal/cachegen/runtype/serialize.go)). Every consumer keys on subKind — don't reintroduce member expansion.
- Types are **deduplicated twice** in [internal/cachegen/runtype/](../internal/cachegen/runtype/) — pointer identity (same `*checker.Type` reached via two paths) AND structural id (two distinct `Type` objects with the same shape). Both collapse to a single cache entry.
...
- When [microsoft/typescript-go#516](https://github.com/microsoft/typescript-go/issues/516) lands with an official transformer/linter API, we replace the shim imports with that API — call sites in [`internal/compiler/program`](../internal/compiler/program/), [`internal/compiler/marker`](../internal/compiler/marker/), and [`internal/compiler/resolver`](../internal/compiler/resolver/) change; everything else (sentinel detection, structural ids, hash dictionary, cache emission, Vite plugin) stays put.
...
The compiler is also the lint engine. `packages/runtypes-devtools/src/eslint/` ships an OXlint JS plugin (that doubles as an ESLint v9 flat-config plugin) whose rules are **pure transport** — no diagnostics logic lives in JS:

- **One resolver pass per file.** The rule pushes the buffer text (`setSources`, same pivot as HMR) and calls `scanFiles` with two opt-in flags: `checkEnrich` (the enrichment-health pass: tag hygiene FT020–FT022 / MD020–MD022 attributed to the mirror's family by [`hygiene.go`](../internal/enrichment/mirror/hygiene.go)'s `FamilyClassifier`, FriendlyText/MockData content FT/MD codes, breadcrumb drift GE002/GE003 — `FamilyEnrich`, produced by [`internal/compiler/resolver/enrichcheck.go`](../internal/compiler/resolver/enrichcheck.go) over [`internal/enrichment/mirror/hygiene.go`](../internal/enrichment/mirror/hygiene.go), [`drift.go`](../internal/enrichment/mirror/drift.go) and the shared [`internal/enrichment/astcheck`](../internal/enrichment/astcheck/) walk; hygiene's comment spans come from one per-file `mirror.Scan` built off the file's EXISTING parse — [`scanTags.go`](../internal/enrichment/mirror/scanTags.go)'s literal-token oracle, shared verbatim by lint, `gen --prune` and the restore-on-reappear index, so all three agree on what a carcass is) and `includeRtDiagnostics` (the RunType render diagnostics — VL010, PJ001, … — rendered exactly as a build would, with the module payload dropped from the wire). The same findings ship from the CLI: `ts-runtypes check <file> --json`, `ts-runtypes gen --check --json`.
- **Routing, not rules.** [`diagnosticRouting.ts`](../packages/runtypes-devtools/src/eslint/diagnosticRouting.ts) maps each wire diagnostic to a rule name: compiler families route by their per-instance severity (`runtypes/error|warn|info`, so oxlint's per-rule severity config keeps the CI error/warn gate faithful), enrich codes route per concern (`no-enrichment-todo`, `no-orphan-carcass`, `enrichment-field`, `enrichment-drift` — the per-family hygiene codes share the concern rules). Messages render from the same catalog the bundler plugin uses: the GENERATED dictionary [`diagnosticCatalog.generated.ts`](../packages/runtypes-devtools/src/diagnosticCatalog.generated.ts), emitted by `pnpm run gen:diag-catalog` from the single Go source of wording ([`internal/diagnostics/messages.go`](../internal/diagnostics/messages.go)), with an unknown-code fallback so a diagnostic is never silently dropped. The mapping is transport-agnostic — an LSP sink can reuse it unchanged.
- **Sync bridge.** Lint rule visitors are synchronous; the resolver clients are Promise-based. [`session.ts`](../packages/runtypes-devtools/src/eslint/session.ts) blocks the rule thread on `Atomics.wait` while a worker thread ([`lint-worker.ts`](../packages/runtypes-devtools/src/eslint/lint-worker.ts)) owns one long-lived `--inline-server` connection for the whole run, memoized per (file, text-hash).
- **Pre-spawned launcher.** oxlint runs the Rust linter inside its Node process and reserves tens of GB of address space once linting starts, after which `fork()` fails on Linux — so the plugin's load is held (top-level await) until the worker has spawned the tiny [`spawn-shim.ts`](../packages/runtypes-devtools/src/eslint/spawn-shim.ts) launcher; the first linted file hands it the real binary + argv over its own pipes. `RT_LINT_PRESPAWN=0` opts out for small hosts.
- **Tag single-source.** The dirty-tag literals (`@todo`, `@rtOrphan`, `@rtOrphanChild`) and the guard signals (`/** @rtType ` marker prefix, `: FriendlyText<` annotation form) live in [`internal/enrichment/mirror/tags.go`](../internal/enrichment/mirror/tags.go); the emitters, the Go detector, and (via `gen:ts-constants`) the JS pre-filters all derive from them, so emitter and detector cannot drift.

## Limitations

- The shim locks us into tsgo's internal API surface. A renovate-driven sync on the tsgolint submodule keeps it current.
- Lint-time analysis builds an inferred single-root Program per file (the `setSources` path): imports resolve from disk, but tsconfig compilerOptions are not applied, so tsconfig-sensitive diagnostics (e.g. the Temporal lib check) can differ from a build. A tsconfig-fidelity lint mode is a possible follow-up (see ROADMAP).
