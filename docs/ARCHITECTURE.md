...
**Noop elision is semantic, not shape-based** ([`internal/compiled/typefns/noop_types.go`](../internal/compiled/typefns/noop_types.go)). `Finalize`'s `"" / "return v"` check only sees inlined code; "is this entry the family identity?" is decided by per-family predicates over the TYPE GRAPH (cycle-safe greatest fixpoint, memoized in `FactsTable`; pj/rj/pjs/fmt implement the optional `NoopTypePredicate` capability today — fmt's default IS identity, so its only falsifiers are a value-transforming format annotation and an `Overrides["fmt"]` cfn redirect on a reachable node). Consumers: (1) the walker's dispatch gate composes around external children the predicate proves identity — no dep call, no import (this is also what collapses circular identity bodies: the cycle re-entry dispatches, gates, and the traversal folds away); (2) JSON composites read their primitives' RENDERED `entrymod.Entry.IsNoop` flags (collected after the family merge) and elide dead bindings — `return JSON.parse(s)` instead of `rjFn(JSON.parse(s))` — and when EVERY binding elides (and the root needs no `[v]` envelope, `rootNeedsDataOnlyWrap`) the composite itself ships as the noop SHORT-FORM tuple: no body, no factory, no imports; the runtime registers the composite family noop from `entryTuple.ts` (native `JSON.stringify` for `je*` tags, `JSON.parse` for `jd*` — NOT the host primitive's value identity, which would corrupt the wire). So a mutate encoder / preserve decoder over a fully JSON-compatible type emits one ~70-byte tuple, and the orphaned pj/rj entries are pruned. SOUNDNESS CONTRACT (one-directional): predicate true ⇒ the emitted body is identity — a false negative only costs bytes, a false positive silently skips a transform. Every arm mirrors its emitter's per-kind dispatch, pinned mechanically by the corpus test ([`internal/resolver/noop_predicate_test.go`](../internal/resolver/noop_predicate_test.go)); when adding a kind or changing an emit arm, keep the predicate in sync (the corpus test fails loudly on the unsound direction). Families without a predicate keep plain dep-call behavior: tb/fb always do real work; the unknown-keys group has none yet; sj — whose compound bodies genuinely differ from native JSON (extras stripped, declaration order) — instead flags its native atomic roots via a `Finalize` byte-match on `return JSON.stringify(v)` (the tb/fb `return Ser`/`return ret` rule), which is what lets a `direct`-strategy composite over an atomic root collapse to the short form too.
